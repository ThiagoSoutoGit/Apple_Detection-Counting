...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if show_label:
            ...
            cv2.putText(image, f'There are {len(bboxes)} apples', (50, 50), cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale*1.5, (0, 0, 255), 1, lineType=cv2.LINE_AA)

    print(f'There are {len(bboxes)} apples in this image')
    return image


def draw_bbox_red_green(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    """
    bboxes: [x_min, y_min, x_max, y_max, probability, cls_id] format coordinates.
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if show_label:
            ...
            cv2.putText(image, bbox_mess, (c1[0], c1[1]-2), cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale, (0, 0, 0), bbox_thick//2, lineType=cv2.LINE_AA)
            rectangle_center = (math.floor((coor[0] + coor[2])/2)-8, math.floor((coor[1] + coor[3])/2)+8)
            cv2.putText(image, str(i+1), rectangle_center, cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale * 2, (0, 0, 255), 2, lineType=cv2.LINE_AA)

    cv2.putText(image, f'There are {len(bboxes)} apples', (50, 50), cv2.FONT_HERSHEY_SIMPLEX,
                fontScale * 2, (255, 0, 0), 2, lineType=cv2.LINE_AA)
    print(f'There are {len(bboxes)} apples in this image')
    return image
